...
### 🅿️ Backlog — split digital-certificates vs credential-management (duplicate content)
`/en/features/digital-certificates` and `/en/features/credential-management` (and their DE equivalents `funktionen/digitale-zertifikate` / `funktionen/zertifikatsverwaltung`) **share one feature content block** (`featureKey="credentialManagement"` in `src/views/features/CredentialManagement.jsx` — both routes point at the same component in `routeConfig`). The shared body is certificate-focused ("Digital Certificates hub", `certificateGrid` hero). Result: two indexable near-duplicate pages (cannibalization risk) and an unavoidable title↔H1 mismatch on whichever page doesn't own the shared block — currently credential-management ships with title "Credential Management System" but H1 "Digital Certificates" (accepted interim; title/meta still target the 7.8k-imp "credential management" term, which is the ranking lever). **Proper fix:** give digital-certificates its own content + view so both pages are distinct & on-target. Needs: `features.digitalCertificates` block (EN `src/messages/en/features.json` **+ DE** `src/messages/de/features.json` — German copy required, else the DE page renders empty), `featureContent.digitalCertificates` (EN; DE falls back via `featx`), `featureMedia.digitalCertificates` (reuse `certificateGrid`; give credentialManagement a dashboard hero e.g. `dashboardAnalytics`/`bulkIssuance`), a new `DigitalCertificates.jsx` view (`featureKey="digitalCertificates"`), and repoint `routeConfig` line ~110 to it. Content task → user-review (bilingual visible copy). Metadata for both pages is already optimized (`seo.digitalCertificates` / `seo.credentialManagement`).
