...
All 136 posts shared ~15 repeating Unsplash/Pexels stock photos (via `getPostImage` in `src/utils/blogImageUtils.js`; `featured_image` was unused except og fallback). Replaced with **per-post branded heroes** (CredSure gradient + topic icon + post title), generated to `public/img/blog/heroes/<slug>.webp` (1200×630, ~20KB), `getPostImage` repointed at them (+ `default.webp` + `onError` fallback; deduped `BlogPost.jsx`'s local copy). `featured_image` set to the absolute hero URL for og:image. Verified live: 12 unique heroes on /en/blog, 0 unsplash. Regenerate script logic lived in `generate-blog-heroes.mjs` (topic detection mirrors the old `topicRules`). **Follow-ups (separate, still stock):** the header **Resources mega-menu** (`src/components/header/ResourcesMegaPanel.jsx` + `messages/*/resources-extra.json`) and `CustomerStorySpotlight.jsx` still use Unsplash; Customer-Success posts could use real photos/logos; German hero titles share the EN image (would need a dynamic per-locale route).
