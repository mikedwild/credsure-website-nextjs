...
### ✅ Done — blog meta sweep (2026-06-24): 85 posts optimized
Audit found **120 of 131 published posts had NO `seo_title`/`seo_description`** (created without them). Fallback = post title (ok) + meta description = body text **truncated mid-word** (poor CTR, no keyword targeting). Fixed the 85 topically-valuable posts (80 via a multi-agent draft workflow + 5 pillar fixes) with keyword-led titles ≤60 (incl `| CredSure`) and 140–160-char descriptions, applied via the partial-update admin API (revision-backed/reversible). **Remaining:** 40 PR/partnership/event posts left on fallback (near-zero SEO value — optional cleanup); **DE meta still missing site-wide on blog posts** (separate pass); `/en/blog/verifiable-credentials-explained` is **NOINDEX/soft-404** (a G3 target sitting un-indexable — investigate whether the post exists/why noindex).
...
### 🔄 In progress — blog internal-link sweep (2026-06-24)
Audit: **120 of 131 posts have ZERO internal links** (only the 11 optimized have any; median 0). Adding 3–5 contextual links/post from a **verified 27-URL allow-list** (strict 200-AND-not-noindex checked; excluded the noindex `verifiable-credentials-explained`). Proposals generated by workflow → strict-recheck every link → user-approved → applied to `content_html`. Body-content change, so per-link verification is mandatory (we shipped a 404 once).
